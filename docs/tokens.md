...
**Отличия от Figma по контрасту (a11y):** `text/tertiary` в glass — 0.5 вместо 0.4; `text/danger` — непрозрачный цвет для текста ошибок (в Figma red @ 0.4). См. `docs/figma-todo.md`.

**Не переносятся, пока нет потребителя или роли:** `color/chart/*`, `color/glass/*` и `color/glass-border/*` (дубли `surface/*` и `border/*`), `placeholder`. См. `docs/figma-todo.md`.
